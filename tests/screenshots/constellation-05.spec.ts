...
test('constellation 05 — Twierdza danych', async ({ page }) => {
  mkdirSync('tests/__results__', { recursive: true });
  await page.setViewportSize({ width: 1536, height: 2200 });
  await page.goto(`${BASE}${SLUG}`);
  await page.waitForLoadState('networkidle');

  const c5 = page.locator('[data-section-type="constellation"][data-constellation-number="05"]');
  await c5.scrollIntoViewIfNeeded();
  await page.waitForTimeout(300);

  await c5.screenshot({
    path: 'tests/__results__/constellation-05.png',
    animations: 'disabled',
  });
});
