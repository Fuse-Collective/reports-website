...
test('area 06 — Nowa podkategoria', async ({ page }) => {
  mkdirSync('tests/__results__', { recursive: true });
  await page.setViewportSize({ width: 1536, height: 1600 });
  await page.goto(`${BASE}${SLUG}`);
  await page.waitForLoadState('networkidle');

  const a6 = page.locator('[data-section-type="analysis"][data-analysis-number="06"]');
  await a6.scrollIntoViewIfNeeded();
  await page.waitForTimeout(200);

  await a6.screenshot({
    path: 'tests/__results__/area-06-full.png',
    animations: 'disabled',
  });
});
